...
import PersistentFileWrapper, { FormidableFile } from "./PersistentFile.js";
import VolatileFileWrapper from "./VolatileFile.js";
...
const DEFAULT_OPTIONS = {
  ...
  maxTotalFileSize: undefined as number | undefined,
  minFileSize: 1,
  ...
  hashAlgorithm: false,
  uploadDir: os.tmpdir(),
  enabledPlugins: [octetstream, querystring, multipart, json] as ((a: any, b: any) => any)[],
  fileWriteStreamHandler: null,
  defaultInvalidName: "invalid-name",
  filter(_part: FormidableFile) {
    return true;
  },
  filename: undefined,
  multiples: false,
...
export type FormidableOptions = Partial<typeof DEFAULT_OPTIONS>;

function hasOwnProp(obj: any, key: string) {
  return Object.prototype.hasOwnProperty.call(obj, key);
}

const decorateForceSequential = function <T extends any[], R>(promiseCreator: (...x: T) => Promise<R>) {
  /* forces a function that returns a promise to be sequential
  useful for fs  for example */
  let lastPromise = Promise.resolve();
  return async function (...x: T): Promise<R> {
...
type ParsingCallback = (err: Error | null, fields: Fields, files: Files) => void;
...
class IncomingForm extends EventEmitter {
  ...
  ended = undefined;
  headers = null;
  type = null as null | undefined;
  bytesExpected = null;
  bytesReceived = 0;
  _parser = null;
  req = null as IncomingMessage | null;
  ...
  fields = {} as Fields;
  constructor(options = {} as FormidableOptions) {
    super();
    ...
    this._setUpRename();
    this.options.enabledPlugins = ([] as ((a: any, b: any) => any)[])
  ...
  async parse(req: IncomingMessage, cb: ParsingCallback) {
    this.req = req;
    let promise;

    // Setup callback first, so we don't miss anything from data events emitted immediately.
    if (!cb) {
      let resolveRef: (value: PromiseLike<[Fields, Files | undefined]> | [Fields, Files | undefined]) => void;
      let rejectRef: (reason?: any) => void;
      promise = new Promise<[Fields, Files | undefined]>((resolve, reject) => {
        resolveRef = resolve;
        rejectRef = reject;
      });

      cb = (err: Error | null, fields: Fields, files?: Files) => {
        if (err) {
          rejectRef(err);
        } else {
    ...
    this.on("field", (name, value) => {
      if (!hasOwnProp(this.fields, name)) {
        ...
        if (Array.isArray(existing)) this.fields[name] = [...existing, value];
        else this.fields[name] = [existing, value];
      }
    });
    ...
    this.on("file", (name, file) => {
      if (!hasOwnProp(files, name)) {
        ...
        if (Array.isArray(existing)) files[name] = [...existing, file];
  ...
  async _handlePart(part) {
    if (part.originalFilename && typeof part.originalFilename !== "string") {
      this._error(
        new FormidableError(`the part.originalFilename should be string when it exists`, errors.filenameNotString)
      );
      return;
    }
    ...
    const fileWrapper = await this._newFile({
      newFilename,
      filepath,
    ...
    fileWrapper.on("error", (err) => {
      this._error(err);
    });
    this.emit("fileBegin", part.name, fileWrapper);
  ...
  async _newFile({ filepath, originalFilename, mimetype, newFilename }) {
    if (this.options.fileWriteStreamHandler) {
      return new VolatileFileWrapper({
        newFilename,
        filepath,
        originalFilename,
        mimetype,
    ...
    return new PersistentFileWrapper({
      newFilename,
      filepath,
      originalFilename,
      mimetype,
    ...
    if (this.options.maxFields !== Infinity) {
      ...
      this.on("field", () => {
        ...
        if (fieldsCount > this.options.maxFields) {
          this._error(
            new FormidableError(`options.maxFields (${this.options.maxFields}) exceeded`, errors.maxFieldsExceeded, 413)
          );
        }
      });
    }
    ...
    if (this.options.maxFiles !== Infinity) {
      ...
      this.on("fileBegin", () => {
        ...
        if (fileCount > this.options.maxFiles) {
          this._error(
            new FormidableError(`options.maxFiles (${this.options.maxFiles}) exceeded`, errors.maxFilesExceeded, 413)
          );
        }
      });
    }
